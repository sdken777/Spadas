...
namespace image_internal
{
    ...
    struct ColorConvertTable
    {
        ...
        void init()
        {
            ...
            {
                RToY[i] = floor(0.29891f * i);
                GToY[i] = floor(0.58661f * i);
                BToY[i] = floor(0.11448f * i);
                
                VToR[i] = round(1.402f * (i - 32768));
                UToG[i] = round(-0.34414f * (i - 32768));
                VToG[i] = round(-0.71414f * (i - 32768));
                UToB[i] = round(1.772f * (i - 32768));
                
                RToU[i] = round(-0.16874f * i);
                GToU[i] = round(-0.33126f * i);
                BToU[i] = round(0.5f * i);
                RToV[i] = round(0.5f * i);
                GToV[i] = round(-0.41869f * i);
                BToV[i] = round(-0.08131f * i);
            }
            inited = TRUE;
        }
    };
...
namespace spadas
{
    ...
    public:
        ...
        ImageVars() : width(0), height(0), rowBytes(0), bytesPerPixel(0), data(NULL), iplImage(NULL)
        {
        }
        ImageVars(UInt data0Size) : width(0), height(0), rowBytes(0), bytesPerPixel(0), data(NULL), data0(data0Size), iplImage(NULL)
        {
        }
        ImageVars(Array<ULong> data00) : width(0), height(0), rowBytes(0), bytesPerPixel(0), data(NULL), data0(data00), iplImage(NULL)
        {
        }
        ~ImageVars()
        {
            if (!iplImage) delete iplImage;
        }
	};
}
