...
void exampleString()
{
	...
	Byte *strBytes = str.bytes(); // UTF8 for any platform / 任何平台都为UTF8
	console::print(SS"bytes: " + String::merge(Array<Byte>(strBytes, str.byteSize()), ","));

	Array<Char> strChars = str.chars(); // It is ANSI for Windows and UTF8 for Linux / Windows下为ANSI，Linux下为UTF8
	console::print(SS"chars: " + String::merge(Array<Byte>((Byte*)strChars.data(), strChars.size()), ","));

	Array<WChar> strWChars = str.wchars(); // It is UTF16 for Windows and UTF32 for Linux / Windows下为UTF16，Linux下为UTF32
	...
	console::print(SS"console::print: " + str); // Spadas printing does not need to worry about character encoding / Spadas打印无需考虑字符编码问题
	...
	// When concatenating strings with operator +, the first object should be initialized by "SS" / 用+连接字符串时，通过SS初始化第一个字符串
	console::print("- 4 -");
