...
void exampleList()
{
	...
	Timer timer;
	List<UInt> list;
	for (UInt i = 0; i < 10000000/* Ten million times / 一千万次 */; i++) list.addToTail(i);
	console::print("Add to tail: " cat timer.check());

	// Efficiency test of linked-list insertion / List插入效率测试
	timer.start();
	for (UInt i = 0; i < 10000000/* Ten million times / 一千万次 */; i++) list.addToHead(i);
	console::print("Add to head: " cat timer.check());

	// Efficiency test of linked-list removing / List移除效率测试
	timer.start();
	for (UInt i = 0; i < 10000000/* Ten million times / 一千万次 */; i++) list.removeHead();
