...
namespace spadas
{
	...
	Type *Interface<Type>::operator ->()
	{
		...
		else return this->vars->implement;
	}
	...
	public:
		...
		ArrayVars(UInt size0) : size(size0), data(new Type[size0]), binded(NULL)
		{ }
	...
	{
		if (!this->vars || index >= this->vars->size)
		{
			SPADAS_ERROR;
			return *(new Type);
		}
	...
	{
		if (!this->vars || !src.vars || this->vars == src.vars)
		{
			SPADAS_ERROR;
			return;
		}
		UInt srcSize = src.vars->size;
		if (srcRegion.offset < 0 || srcRegion.offset + srcRegion.size > srcSize)
		{
			SPADAS_ERROR;
			return;
		}
		if (thisOffset + srcRegion.size > this->vars->size)
		{
			SPADAS_ERROR;
			return;
		}
	...
	template <typename Type>
	void ListNode<Type>::insertNext(ListNode<Type> newNode)
	{
		newNode.cutPrevious();
		newNode.cutNext();
		if (this->vars->nextVars == 0) joinNext(newNode);
		else
		{
			ListNode<Type> nextNode(this->vars->nextVars);
			joinNext(newNode);
			nextNode.joinPrevious(newNode);
		}
	}

	template <typename Type>
	void ListNode<Type>::insertPrevious(ListNode<Type> newNode)
	{
		newNode.cutPrevious();
		newNode.cutNext();
		if (this->vars->previousVars == 0) joinPrevious(newNode);
		else
		{
			ListNode<Type> previousNode(this->vars->previousVars);
			joinPrevious(newNode);
			previousNode.joinNext(newNode);
		}
	}
	...
	void ListNode<Type>::removeSelf()
	{
		if (this->vars->nextVars == 0)
		{
			if (this->vars->previousVars != 0) cutPrevious();
		}
		else //vars->nextVars != 0
		{
			...
			else //vars->previousVars != 0
			{
				...
				previousNode.joinNext(nextNode);
			}
		}
	}
	...
	ListNode<Type> ListNode<Type>::cloneList()
	{
		...
		while (TRUE)
		{
			...
			ListNode<Type> dstNewNode(srcCurrent.value());
			dstCurrent.insertPrevious(dstNewNode);
			dstCurrent = dstNewNode;
		}
		srcCurrent = *this;
		dstCurrent = dst;
		while (TRUE)
		{
			...
			ListNode<Type> dstNewNode(srcCurrent.value());
			dstCurrent.insertNext(dstNewNode);
			dstCurrent = dstNewNode;
		}
		return dst;
	}
	...
	void ListNode<Type>::collapse()
	{
		...
		while (TRUE)
		{
			...
			currentNode = ListNode<Type>(currentNode.vars->nextVars);
			currentNode.cutPrevious();
		}
		currentNode = ListNode<Type>(*this);
		while (TRUE)
		{
			if (currentNode.vars->previousVars == 0) break;
			currentNode = ListNode<Type>(currentNode.vars->previousVars);
			currentNode.cutNext();
		}
	}
	...
	TreeNode<Type> TreeNode<Type>::cloneTree()
	{
		...
		for (UInt i = 0; i < this->vars->nLeaves; i++)
		{
			node.addLeaf(leafAt(i).cloneTree());
		}
		return node;
	}
	...
	public:
		...
		TreeNode<Array<Type> > root;
		Type defaultValue;
		ArrayXVars(UInt segSize, UInt power) : size(0), segmentSize(segSize), segmentSizePower(power), segmentSizeMask(segSize - 1), lastAccessedSegment(1), root(Array<Type>(segSize))
		{
			lastAccessedData = root.value().data();
		}
		ArrayXVars(UInt segSize, UInt power, Type& defaultVal) : size(0), segmentSize(segSize), segmentSizePower(power), segmentSizeMask(segSize - 1), lastAccessedSegment(1), root(Array<Type>(segSize, defaultVal)), defaultValue(defaultVal)
		{
			lastAccessedData = root.value().data();
		}
		~ArrayXVars()
	...
	Type& ArrayX<Type>::operator [](UInt index)
	{
		...
		for (; depth < 30; depth++)
		{
			if ((1 << (depth + 1)) >(Int)segment) break;
		}
		TreeNode<Array<Type> > currentNode = this->vars->root;
		for (Int i = depth - 1; i >= 0; i--)
		{
			if (currentNode.nLeaves() == 0)
			{
				currentNode.addLeaf(Array<Type>(this->vars->segmentSize, this->vars->defaultValue));
				currentNode.addLeaf(Array<Type>(this->vars->segmentSize, this->vars->defaultValue));
			}
			currentNode = currentNode.leafAt((segment & (1 << i)) >> i);
		}
		this->vars->size = math::max(this->vars->size, index + 1);
		this->vars->lastAccessedSegment = segment;
		this->vars->lastAccessedData = currentNode.value().data();
		return this->vars->lastAccessedData[localIndex];
	}
	...
	void ArrayX<Type>::setSize(UInt size)
	{
		...
		if (size > this->vars->size) operator[](lastIndex) = this->vars->defaultValue;
		else if (size < this->vars->size)
		{
			...
			if (this->vars->lastAccessedSegment > lastSegment)
			{
				...
				this->vars->lastAccessedData = this->vars->root.value().data();
			}
		}
	}

	template<typename Type>
	void ArrayX<Type>::validateInSize(TreeNode<Array<Type> > node, UInt segment, UInt lastSegment, UInt lastLocalIndex)
	{
		...
		{
			UInt i = (segment == lastSegment) ? (lastLocalIndex + 1) : 0;
			Type *data = node.value().data();
			for (; i < this->vars->segmentSize; i++)
			{
				data[i] = this->vars->defaultValue;
			}
			if (node.nLeaves() != 0)
			{
				TreeNode<Array<Type> > leafNode = node.leafAt(1);
	...
	template<typename Type>
	void ArrayX<Type>::copyRegion(TreeNode<Array<Type> > *nodePtr, UInt segment, Type *dst, UInt firstIndex, UInt lastIndex, UInt firstSegment, UInt lastSegment)
	{
		if (segment >= firstSegment && segment <= lastSegment)
		{
			...
			if (nodePtr)
			{
				Type *srcPtr = nodePtr->value().data();
		...
		if (segment < lastSegment)
		{
			...
			{
				TreeNode<Array<Type> > leafNode0 = nodePtr->leafAt(0);
				TreeNode<Array<Type> > leafNode1 = nodePtr->leafAt(1);
	...
	void ListElem<Type>::insertPrevious(Type val)
	{
		...
		ListNode<Type> prevNode(val);
		this->vars->node.insertPrevious(prevNode);
		this->vars->prevNode = prevNode;
		this->vars->prevValid = TRUE;
		this->vars->prevIndex = this->vars->index;
		this->vars->index++;
	...
	{
		if (!this->vars->valid) return;
		ListNode<Type> nextNode(val);
		this->vars->node.insertNext(nextNode);
		this->vars->nextNode = nextNode;
		this->vars->nextValid = TRUE;
		this->vars->nextIndex = this->vars->index + 1;
		this->vars->listVars->size++;
	}
	...
	void Stream<Type>::enqueue(Array<Type> newElements)
	{
		...
		if (!this->vars->discardable)
		{
			if (elemSize > this->vars->capacity)
			{
				SPADAS_ERROR;
				return;
			}
	...
	ValueType& Map<KeyType, ValueType>::operator [](KeyType key)
	{
		...
		else
		{
			...
			while (TRUE)
			{
				...
				{
					KeyValue<KeyType, ValueType> pair(key, ValueType());
					ListNode<KeyValue<KeyType, ValueType> > newNode(pair);
					node.joinNext(newNode);
					this->vars->size++;
