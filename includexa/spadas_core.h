...
namespace spadas
{
	...
	private:
		Bool isNull() { return FALSE; }
		Bool isValid() { return FALSE; }
		void validateInSize(TreeNode<Array<Type> >, UInt, UInt, UInt);
		void copyRegion(TreeNode<Array<Type> >*, UInt, Type*, UInt, UInt, UInt, UInt);
	};
	...
	public:
		...
		/// 取得首个元素（使用该对象时，不可调用链表的add/remove/clear等方法，或再创建其他ListElem）
		ListElem<Type> head();

		/// 取得末尾元素（使用该对象时，不可调用链表的add/remove/clear等方法，或再创建其他ListElem）
		ListElem<Type> tail();
